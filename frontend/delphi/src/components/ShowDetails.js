...
const ShowDetails = ({ show }) => {
    const { title, director, year, runtime, genre, plot, awards, imdb_rating } = show;
    ...
    const getImageUrl = () => {
        return show.poster_url !== "N/A" ? show.poster_url : "/default-movie-poster.png"
    }

    const alreadyExists = () => {
        const url = "http://127.0.0.1:8000/profile/"
        const authToken = getCookie("AUTH_TOKEN");
        console.log("Checking if show already exists in library.");
        ...
        axios.get(url, {
            ...
            if (response.data.shows.includes(show.imdb_id)) {
                setSuccessMsg(true);
    ...
    const removeFromLibrary = (e) => {
        e.preventDefault();
        const url = "http://127.0.0.1:8000/profile/shows/remove/";
        const authToken = getCookie("AUTH_TOKEN");
    ...
    const addToLibrary = (e) => {
        e.preventDefault();
        const url = "http://127.0.0.1:8000/profile/shows/add/"
        const authToken = getCookie("AUTH_TOKEN");
    ...
    return (
        <div className="movie-details container">
            ...
            <div className="right-section">
                ...
                <a className='imdb-link' href={`https://www.imdb.com/title/${show.imdb_id}/`} target="_blank" rel='noreferrer'>
                    See on IMDB
                </a>
            </div>
